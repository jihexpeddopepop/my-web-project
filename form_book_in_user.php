...
<!-- Form Name -->
<br>
<p align="right"><a href="insert_user.php" > <button type="button"  class="btn btn-danger"> ย้อนกลับ </button></a></p>
<center><legend><h2>หนังสือเข้า</h2></legend></center> 
...
<!-- Text input-->
<div class="form-group">
  <label class="col-md-4 control-label" for="book_detail_dept2">หน่วยงานที่ส่งหนังสือ</label>  
  <div class="col-md-4">
  <select name="txt_book_detail_dept" id="txt_book_detail_dept">
<?php
	  $query_dept = "select * from book_dept  ";
    $result_dept = mysqli_query($con, $query_dept);

	  echo "<option value='all' selected='selected'>-- เลือกทั้งหมด --</option>";
    while($row_dept = mysqli_fetch_assoc($result_dept)) {
    echo "<option value=".$row_dept['book_dept_id'].">".$row_dept['book_dept_name']."</option>";
      }
	  ?>
    </select></div>
</div>
...
<table class="table table-sm table-dark" width="85%" align="center">
    <thead >
      ...
      <th><h4><center scope="col">หนังสือเข้า</h4></th></center>
      <th><h4><center scope="col">หน่วยงานที่ส่งหนังสือ</h4></th></center>
    ...
    <?php
    if($_POST["txt_book_detail_dept"] =='all')
    {
      ...
      where b1.book_detail_dept2='{$_SESSION["booK_dept_id"]}' 
      and b1.book_detail_date between  '{$_POST['date_1']}'  
      and  '{$_POST['date_2']}' ";
    ...
    else
    {
      ...
      LEFT OUTER JOIN book_dept b3 ON b3.book_dept_id = b1.book_detail_dept2
      where b1.book_detail_dept2='{$_SESSION["booK_dept_id"]}' 
      and b1.book_detail_dept1 = '{$_POST["txt_book_detail_dept"]}'
...
if($_POST["txt_book_detail_dept"] =='all')
{
  $query_pdf = "SELECT b1.book_detail_id, b1.book_detail_no, b1.book_detail_title, b1.book_detail_detail,b2.book_dept_name AS deptname1,b3.book_dept_name AS deptname2
  , b1.book_detail_date, b1.book_detail_file 
  FROM book_detail b1 
  LEFT OUTER JOIN book_dept b2 ON b2.book_dept_id = b1.book_detail_dept1
  LEFT OUTER JOIN book_dept b3 ON b3.book_dept_id = b1.book_detail_dept2
...
$objQuery = mysqli_query($con,$query_pdf);
// $resultData = array();
...
<p align="center"> &nbsp; 
  ...
  <a href="export_excel_in_user.php?d1=<?php echo $_POST['date_1'];
   ?>&d2=<?php echo $_POST['date_2']; ?>&dep2=<?php echo $_POST['txt_book_detail_dept'];
